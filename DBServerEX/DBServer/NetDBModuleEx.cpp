...
int CNetDBModuleEx::SavePlayerData(LPCSTR pAccount, DWORD dwServerID, SFixData &bdata, SWareHouses *wdata)
{
	...
	DWORD guideID = 1;//新手引导 千位为引导类型,以下为哪一步
...
{
	BYTE *pBuffer = pMsg->pBuffer;

	QueryBind< 1 > args;
	/*args[0].BindBlob(&stFaction, sizeof(SaveFactionData_New));*/
	args[0].BindBlob(pMsg->pBuffer, pMsg->wLenth);
...
int CNetDBModuleEx::GetScriptData(const DNID dwSID,struct SQGetScriptData *pMsg)
{
	...
	while (qr.FetchResult(bind) == 0)
	{
		...
		memcpy(sqscriptdata.pBuffer, unpakData, bind[0].Length());
		sqscriptdata.wLenth = bind[0].Length();
		SendMessage(dwSID, &sqscriptdata, sizeof(SAGetScriptData));
	}
	return 0;
}
