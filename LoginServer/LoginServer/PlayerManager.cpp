...
BOOL CPlayerManager::SavePlayerData(LPCSTR account, SFixNewData &playerData, int storeFlag)
{
	// 可能是试玩角色的数据，就不用保存了。。。
	if (memcmp(account, "游客", 4) == 0)
		return TRUE;
	...
	{
		rfalse(1, 0, "detected a overflowed data!");
		Globals::TraceMemory(&playerData, 0, sizeof(SFixNewData), "Overflow.mem");
		return TRUE;
	}
	...
	LPVOID outData = NULL;
	size_t outSize = 0;
	BOOL result = TryEncoding_NilBuffer(&playerData, sizeof(SFixNewData), outData, outSize);
	if (result && (outSize < sizeof(SFixNewData) - 32))
	{
		// 压缩成功后，帐号的数据还得重新处理过！
		LPDWORD dataBuf = (LPDWORD)&msg.PlayerData;
		*dataBuf = (DWORD)outSize;
...
BOOL CPlayerManager::AssignPlayer(DNID dnidClient, SFixData *data)
{
	...
	memcpy(&player.fixData, data, sizeof(SFixData));
...
BOOL CPlayerManager::GetPlayerData(LPCSTR account, DWORD gid, SFixData *data, SPlayerTempData *tdata)
{
	...
	memcpy(data, &it->second.fixData, sizeof(SFixData));
