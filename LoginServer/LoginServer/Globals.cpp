...
{
    return GetApp()->m_dbCenterClient.client.SendMessage( msg, size );
}
...
BOOL Globals::SendToDBCenterEx( DWORD dbcId, void *msg, size_t size )
{
    ...
    return it->second->client.SendMessage( msg, size );
}

BOOL Globals::SendToDBServer( void *msg, size_t size )
{
    return GetApp()->m_DatabaseClient.client.SendMessage( msg,size );
}

BOOL Globals::SendToGameServer( DNID dnid, LPCVOID msg, size_t size )
{
    return GetApp()->m_ServerManager.server.SendMessage( dnid, msg, size );
}
