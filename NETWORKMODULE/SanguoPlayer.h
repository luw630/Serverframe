...
struct SSanGuoFixData : SSanGuoPlayerFixData
{
...
///@brief struct 玩家数据
struct SSanGuoNewPlayerData
{
	BYTE   bData[LUAPLAYERBUFFER];
	DWORD  dBufferLenth; //序列化以后的长度
	enum SAVESTATE		///< 角色服务器上的状态
	{
		ST_LOGIN = 1,	// 刚刚登入
		ST_LOGOUT,		// 退出
...
///@brief struct 固定的玩家属性，会保存下来的数据	
struct SFixData : SSanGuoNewPlayerData
{ 
	static DWORD GetVersion() { return (sizeof(SFixData) << 16 | VERSIONID); };
};

///@brief struct 固定的玩家属性，会保存下来的数据	
struct SFixNewData : SSanGuoNewPlayerData
{
	static DWORD GetVersion() { return (sizeof(SFixNewData) << 16 | VERSIONID); };
};
