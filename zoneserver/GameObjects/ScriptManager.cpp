...
int CScriptManager::L_SendFactionList(lua_State *L)
{
	...
	if (lua_istable(L, 2))
	{
		...
		for (int i = 0; i < nlenth; i++)
		{
			...
			{
				rfalse(2, 1, "SendFactionList to long %d", nlenth);
				break;
			}
		}
...
int CScriptManager::L_Logout(lua_State *L)
{
	CNewPlayer *pPlayer = NULL;
	if (lua_isnumber(L, 1))
	{
		DWORD sid = static_cast<DWORD>(lua_tonumber(L, 1));
		pPlayer = (CNewPlayer*)GetPlayerBySID(sid)->DynamicCast(IID_NEWPLAYER);
	}
	if (!pPlayer)
	{
		return 0;
	...
	if (lua_istable(L, 2))
	{
		BYTE   PBuff[LUAPLAYERBUFFER];
		DWORD  dBufflenth = 0;
		int cur1 = luaEx_serialize(L, 2, PBuff, LUAPLAYERBUFFER);
		if (cur1 < 0)
		{
			rfalse(2, 1, " LUAPLAYERBUFFER Data size was to long  %d ", cur1);
			return 0 ;
		}
		dBufflenth = cur1;
		pPlayer->Logout(PBuff, dBufflenth);
		return 1;
	}
	return 0;
}
...
int CScriptManager::L_SendScriptData(lua_State *L)
{
	...
	if (lua_istable(L,1))
	{
		SQScriptData   scriptdata;
		memset(scriptdata.pBuffer, 0, BIGDATA);
		int cur1 = luaEx_serialize(L, 1, scriptdata.pBuffer, BIGDATA);
		if (cur1 < 0)return 0;
		scriptdata.wLenth = cur1;
		SendToLoginServer(&scriptdata, sizeof(SQScriptData));
...
int CScriptManager::L_GetScriptData(lua_State *L)
{
	SQGetScriptData  qgetdata;
	SendToLoginServer(&qgetdata, sizeof(SQGetScriptData));
	return 1;
}
