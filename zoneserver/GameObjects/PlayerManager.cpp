...
IObject* CPlayerManager::CreateObject(const __int32 IID)
{
	if (IID == IID_PLAYER)
		return new CPlayer();
	else if (IID_NEWPLAYER)
	{
		return new CNewPlayer();
	}
...
BOOL CALLBACK KickPlayer( LPIObject &Player, LPARAM )
{
	...
	if (CNewPlayer *pPlayer = (CNewPlayer*)Player->DynamicCast(IID_NEWPLAYER))
	{
...
//     if (pObj->DynamicCast(IID_PLAYER) == NULL)
//         return FALSE;
	if (pObj->DynamicCast(IID_NEWPLAYER) == NULL)
		return FALSE;
...
BOOL  CALLBACK ChecklivePlayer( LPIObject &Player, DWORD &number )
{
	CNewPlayer *pdest = (CNewPlayer*)Player->DynamicCast(IID_NEWPLAYER);
	if (pdest && GetGW())
	{
