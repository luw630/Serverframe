...
extern dwt::stringkey<char[256]> scriptPath;
//#define GAME_FPS                5
...
// 	CSingleRank::GetInstance().Initialized();
	...
	// 载入脚本
	if (dwt::strlen(scriptPath,256) == 0)
	{
		g_Script.SetPath("Script\\");
	}
	else
	{
		g_Script.SetPath(scriptPath);
...
BOOL CGameWorld::SetPlayerData(DNID dnidClient, DWORD GID, SFixData *Data, SPlayerTempData *pTempData)
{
	...
	CNewPlayer *pPlayer = (CNewPlayer *)GetPlayerByDnid(dnidClient)->DynamicCast(IID_NEWPLAYER);
	if (!pPlayer)
	{
		rfalse(4, 1, "GetPlayerByDnid NULL %d",dnidClient);
		//这里在登录，特别是玩家较多同时战斗中会出现无法取得玩家的情况，按理说应该不会出现的，所有尝试使用下面的方法，但是下面的方法比较慢
		pPlayer = (CNewPlayer*)GetObjectByGID(GID)->DynamicCast(IID_NEWPLAYER);
		if (!pPlayer)
		{
	...
	if (obj&&obj->DynamicCast(IID_NEWPLAYER))
	{
...
//     Param.dwVersion  = 1;
	...
	if (pObj)
    {
		...
		CNewPlayer *pPlayer = (CNewPlayer *)pObj->DynamicCast(IID_NEWPLAYER);
		if (!pPlayer)
		{
			MY_ASSERT(0);
			return 0;
...
//         pPlayer->SendMyState();
	...
	pObj = CPlayerManager::GenerateObject(IID_NEWPLAYER, GID, (LPARAM)&Param);
	if (pObj)
	{
		//CPlayer *pPlayer = (CPlayer *)pObj->DynamicCast(IID_PLAYER);
		CNewPlayer *pPlayer = (CNewPlayer *)pObj->DynamicCast(IID_NEWPLAYER);
...
BOOL CGameWorld::OnLogout(DNID dnidClient, struct SQLogoutMsg *pMsg, CNewPlayer *pPlayer)
{
	if (0 == dnidClient || !pMsg || !pPlayer)
		return FALSE;
	//三国项目 global ID没有用处，故把此段代码屏蔽
